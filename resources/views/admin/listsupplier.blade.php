...
<title>Supplier Registry | Admin Dashboard</title>
  ...
  <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 border-b pb-4">
    ...
    <div class="flex gap-3">
      ...
      <a href="{{ route('admin.suppliers.create') }}">
        <button class="flex items-center justify-center px-4 py-2 bg-leaf text-white font-semibold rounded-full text-sm hover:bg-earth transition duration-200 shadow-sm">
      ...
      <table class="min-w-full divide-y divide-gray-200 bg-white table-auto">
        <thead>
          <tr class="bg-leaf text-white font-bold text-sm uppercase tracking-wider">
            ...
            <th class="px-4 py-3 text-left">Category supplier</th>
            <th class="px-4 py-3 text-center">Delivery Time (days)</th>
        ...
        <tbody class="divide-y divide-gray-100">
          ...
          <tr class="table-row-hover transition-all duration-200">
            ...
            <td class="px-4 py-4 text-sm font-semibold text-earth">{{ $supplier->supplier_name }}</td>
            <td class="px-4 py-4 text-sm">
              <div class="text-gray-700">{{ $supplier->email }}</div>
              <div class="text-xs text-gray-500">{{ $supplier->phone_number }}</div>
            </td>
            <td class="px-4 py-4 text-sm text-gray-600 break-words w-64">{{ $supplier->address }}</td>
            <td class="px-4 py-4 text-sm text-gray-600 break-words w-64">{{ ucfirst($supplier->supplier_type) }}</td>
            <td class="px-4 py-4 text-sm text-gray-600 text-center">{{ $supplier->delivery_time }}</td>
            <td class="px-4 py-4 text-center text-sm font-medium space-x-3">
              <a href="{{ route('admin.suppliers.edit', $supplier->supplier_id) }}" class="text-leaf hover:text-earth transition duration-200">
                <i class="fa-solid fa-user-pen text-lg"></i>
              </a>
              <form action="{{ route('admin.suppliers.destroy', $supplier->supplier_id) }}" method="POST" class="inline">
                @csrf
                @method('DELETE')
                <button type="submit" class="text-terracotta hover:text-red-700 transition duration-200" onclick="return confirm('Are you sure you want to permanently delete this supplier?')">
          ...
          <tr>
            <td colspan="6" class="px-6 py-12 text-center text-gray-500 text-xl font-medium">
              <i class="fa-solid fa-truck-field text-4xl mb-3 text-terracotta/50"></i>
